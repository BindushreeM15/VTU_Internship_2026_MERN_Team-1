...
const KYC_DOCS = [
  {
    ...
    description: "GST registration certificate of your company",
    required:    true,
  },
  {
    key:         "companyPan",
    label:       "Company PAN Card",
    description: "PAN card issued to the company (not personal)",
    required:    true,
  },
  {
    key:         "incorporationCert",
    label:       "Certificate of Incorporation / Partnership Deed",
    description: "Proof that your company is legally registered",
    required:    true,
  },
  {
    key:         "directorId",
    label:       "Director / Owner ID",
    description: "Personal PAN or Aadhaar of the director / owner",
    required:    false,
  },
  {
    key:         "addressProof",
    label:       "Company Address Proof",
    description: "Utility bill or rent agreement of the company office",
    required:    false,
  },
];
...
const STATUS_CONFIG = {
  unsubmitted: {
    color: "bg-yellow-500/10 text-yellow-600 border-yellow-400/30",
    icon:  AlertCircle,
    label: "Not Submitted",
  },
  under_review: {
  ...
  verified: {
    color: "bg-green-500/10 text-green-600 border-green-400/30",
    icon:  CheckCircle2,
    label: "Verified",
  },
  rejected: {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 space-y-8">
      ...
      <div className="space-y-1">
        <p className="text-primary text-xs tracking-[3px] uppercase font-medium">Company Verification</p>
        <h1 className="display-font text-4xl font-bold text-foreground">
        ...
        <CardContent className="pt-5 flex items-start gap-4">
          ...
          <div className="space-y-1">
            <p className="font-semibold text-sm">
              Verification Status: <span className="uppercase tracking-wider">{statusConfig.label}</span>
            </p>
            {status === "unsubmitted" && (
            ...
            {status === "under_review" && (
              <p className="text-xs">
                Documents submitted on {new Date(kycData.kycSubmittedAt).toLocaleDateString("en-IN", {
            ...
            {status === "verified" && (
              <p className="text-xs">
                Your company is verified. You can now create and manage projects.
              </p>
            )}
            {status === "rejected" && (
              <div className="space-y-2">
                <p className="text-xs font-medium">Reason for rejection:</p>
                <p className="text-xs bg-red-500/5 border border-red-400/20 rounded px-3 py-2">
      ...
      {/* Under review — just show docs */}
      {status === "under_review" && (
        <Card className="border-border bg-card/70">
          <CardHeader>
            <CardTitle className="text-base">Submitted Documents</CardTitle>
            <CardDescription>The following documents are being reviewed.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {kycData.kycDocuments?.map((doc) => (
              <div key={doc.docType} className="flex items-center justify-between py-2 border-b border-border last:border-0">
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Upload at least 3 of the 5 documents. Accepted formats: PDF, JPG, PNG (max 10 MB each).
                  </CardDescription>
                </div>
            ...
            <CardContent className="pt-5 space-y-4">
              {KYC_DOCS.map((doc) => (
                <div key={doc.key} className="space-y-1.5">
                  ...
                  <div
                    className={`relative flex items-center gap-3 border rounded-md px-4 py-3 transition-colors
                      ${files[doc.key]
              ...
              {uploadedCount < 3 && (
                <Alert variant="default" className="border-yellow-400/30 bg-yellow-500/5">
              ...
              <div className="flex gap-3 pt-2">
                <Button
                  type="submit"
                  disabled={isSubmitting || uploadedCount < 3}
                  className="gap-2"
                >
                  {isSubmitting ? (
                    <><Loader2 className="h-4 w-4 animate-spin" /> Submitting...</>
                  ) : (
                    <>{status === "rejected" ? <RefreshCw className="h-4 w-4" /> : <Upload className="h-4 w-4" />}
