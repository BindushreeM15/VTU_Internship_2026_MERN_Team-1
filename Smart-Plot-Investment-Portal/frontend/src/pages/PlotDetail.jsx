...
function ProjectImageSlideshow({ sketchImage, projectImages, companyName }) {
  ...
  if (!allImages.length) {
    ...
    return (
      <div
        className="w-full h-64 rounded-2xl flex flex-col items-center justify-center border"
        style={{ background: "color-mix(in srgb, var(--primary) 10%, var(--muted))", borderColor: "var(--border)" }}
      >
        <div
          className="w-16 h-16 rounded-full border-2 flex items-center justify-center text-xl font-bold mb-2"
      ...
      <div className="relative rounded-2xl overflow-hidden h-64 bg-muted group">
        ...
        {allImages.length > 1 && (
          <>
            <button
              onClick={() => go(-1)}
              className="absolute left-2.5 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
            >
              <ChevronLeft className="h-4 w-4 text-white" />
            </button>
            <button
              onClick={() => go(1)}
              className="absolute right-2.5 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
            >
...
function ProximitySection({ plot }) {
  const items = [
    { key: "distanceToMetro", emoji: "🚇", label: "Metro Station", color: "text-blue-600 dark:text-blue-400" },
    { key: "distanceToHighway", emoji: "🛣", label: "Highway", color: "text-orange-600 dark:text-orange-400" },
    { key: "distanceToSchool", emoji: "🏫", label: "School", color: "text-green-600 dark:text-green-400" },
    { key: "distanceToHospital", emoji: "🏥", label: "Hospital", color: "text-red-600 dark:text-red-400" },
        ...
        {items.map((item) => (
          ...
          >
            ...
            <div>
              ...
              <p className="text-sm font-bold text-foreground">{plot[item.key]} km</p>
            </div>
          </div>
        ))}
      </div>
