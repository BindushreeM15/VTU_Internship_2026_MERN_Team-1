  ...
  return (
    <div className="rounded-xl border p-5 space-y-4" style={{ background:"var(--card)", borderColor:"var(--border)" }}>
      ...
      <div className="flex flex-wrap gap-2">
        {badges.map(b => (
          <span key={b.key}
            className={`trust-badge ${b.doc ? "" : "trust-badge-pending"}`}>
...
function PlotCard({ plot, projectId, isLoggedIn }) {
  ...
  return (
    <div onClick={handleClick}
      className="card-sp cursor-pointer p-4 space-y-3 group">
            ...
            {project.locationLink && (
              <Button variant="outline" asChild className="gap-2">
                <a href={project.locationLink} target="_blank" rel="noreferrer">
                  <MapPin className="h-4 w-4" /> Map <ExternalLink className="h-3.5 w-3.5" />
                </a>
              </Button>
            )}
            {!isLoggedIn && (
