...
function ProjectSlideshow({ sketchImage, projectImages, companyName }) {
  ...
  if (!allImages.length) {
    ...
    return (
      <div
        className="w-full h-80 rounded-2xl flex flex-col items-center justify-center border"
        style={{ background: "color-mix(in srgb, var(--primary) 10%, var(--muted))", borderColor: "var(--border)" }}
      >
        <div
          className="w-20 h-20 rounded-full border-2 flex items-center justify-center text-2xl font-bold mb-3"
    ...
    <div className="space-y-3">
      <div className="relative rounded-2xl overflow-hidden h-80 bg-muted group">
        ...
        {allImages.length > 1 && (
          <>
            <button
              onClick={() => { clearInterval(timer.current); setIdx((i) => (i - 1 + allImages.length) % allImages.length); start(); }}
            ...
            <button
              onClick={() => { clearInterval(timer.current); setIdx((i) => (i + 1) % allImages.length); start(); }}
...
const PROXIMITY_ITEMS = [
  { key: "distanceToMetro", emoji: "🚇", label: "Metro" },
  { key: "distanceToHighway", emoji: "🛣", label: "Highway" },
  { key: "distanceToSchool", emoji: "🏫", label: "School" },
  { key: "distanceToHospital", emoji: "🏥", label: "Hospital" },
];
...
function PlotCard({ plot, projectId, isLoggedIn }) {
  ...
  const proximityItems = PROXIMITY_ITEMS.filter((item) => plot[item.key] != null);
  ...
  return (
    <div onClick={handleClick} className="card-sp cursor-pointer p-4 space-y-3 group">
      ...
      <div className="grid grid-cols-2 gap-1 text-xs text-muted-foreground">
        ...
        {plot.dimensions && (
          <span className="col-span-2">Dim: <span className="text-foreground font-medium">{plot.dimensions}</span></span>
        )}
        {plot.cornerPlot && <span className="text-primary font-semibold col-span-2">✦ Corner Plot</span>}
      </div>

      {/* Proximity chips — compact display */}
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-5">
          <div>
            ...
            <h1
              className="display-font font-bold text-foreground leading-tight"
          ...
          <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
            <span className="flex items-center gap-1.5">
              <Building2 className="h-4 w-4 text-primary" />{project.builderId?.companyName}
            </span>
            <span className="flex items-center gap-1.5">
          ...
          {minPrice !== null && (
            ...
            >
              ...
              <p className="display-font text-2xl font-bold text-foreground">
                ...
                {maxPrice !== minPrice && (
                  <span className="text-muted-foreground font-normal text-lg"> – {formatPrice(maxPrice)}</span>
                )}
              </p>
              <p className="text-xs text-muted-foreground">
                {plotStats.total} plots · {plotStats.available} available
              </p>
            </div>
          )}
          ...
          <div className="flex gap-3 flex-wrap">
            ...
            {!isLoggedIn && (
              <Link to="/login">
                <Button variant="outline" className="gap-2">Login to interact</Button>
              </Link>
            )}
          </div>
        </div>
      ...
      <div className="space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              {plotStats.available} available · {plotStats.reserved} reserved · {plotStats.sold} sold
            </p>
          </div>
        ...
        {filteredPlots.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredPlots.map((p) => (
              <PlotCard key={p._id} plot={p} projectId={projectId} isLoggedIn={isLoggedIn} />
            ))}
          </div>
        )}
      </div>
