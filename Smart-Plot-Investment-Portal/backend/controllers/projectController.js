...
exports.getBuilderProjects = async (req, res) => {
    try {
        const projects = await Project.find({ builderId: req.user.id }).sort({
            createdAt: -1,
        });
        res.json({ projects });
    } catch (error) {
...
exports.updateProject = async (req, res) => {
    try {
        ...
        if (project.projectStatus === "under_review") {
            return res
                .status(400)
                .json({ error: "Cannot edit a project under review" });
        }
        if (project.projectStatus === "completed") {
            return res
                .status(400)
                .json({ error: "Cannot edit a completed project" });
        }
        ...
        if (projectStatus !== undefined) {
            ...
            if (
                !(allowed[project.projectStatus] || []).includes(projectStatus)
            ) {
                return res.status(400).json({
        ...
        if (newImages.length) {
            ...
            if (current + newImages.length > 5) {
                return res
                    .status(400)
                    .json({ error: "Maximum 5 project images allowed" });
            }
...
exports.deleteProject = async (req, res) => {
    try {
        ...
        if (project.projectStatus === "completed") {
            return res
                .status(400)
                .json({ error: "Cannot delete a completed project" });
        }
...
async function _cleanupProjectFiles(project) {
    ...
    if (project.sketchImage?.publicId) {
        try {
            await fs.unlink(
                path.join(uploadsDir, project.sketchImage.publicId),
            );
        } catch (_) {}
    }
    ...
    if (project.kathaDocument?.publicId) {
        try {
            await fs.unlink(
                path.join(uploadsDir, project.kathaDocument.publicId),
            );
        } catch (_) {}
    }
}
...
exports.submitProjectForReview = async (req, res) => {
    try {
        ...
        if (!projectId || !mongoose.Types.ObjectId.isValid(projectId)) {
            return res
                .status(400)
                .json({ error: "A valid Project ID is required" });
        }
        if (!kathaType)
            return res.status(400).json({ error: "Katha type is required" });
        ...
        const missingDocs = REQUIRED_PROJECT_DOCS.filter(
            (d) => !submittedTypes.includes(d),
        );
...
exports.addPlot = async (req, res) => {
    try {
        ...
        if (!["verified", "active"].includes(project.projectStatus)) {
            return res.status(400).json({
                error: "Plots can only be added to verified or active projects",
            });
        }
        ...
        if (exists)
            return res
                .status(400)
                .json({ error: "Plot number already exists in this project" });
        ...
        const plot = new Plot({
            ...
            distanceToMetro:
                distanceToMetro !== "" && distanceToMetro != null
                    ? Number(distanceToMetro)
                    : null,
            distanceToHighway:
                distanceToHighway !== "" && distanceToHighway != null
                    ? Number(distanceToHighway)
                    : null,
            distanceToSchool:
                distanceToSchool !== "" && distanceToSchool != null
                    ? Number(distanceToSchool)
                    : null,
            distanceToHospital:
                distanceToHospital !== "" && distanceToHospital != null
                    ? Number(distanceToHospital)
                    : null,
        });
...
exports.getProjectPlots = async (req, res) => {
    try {
        ...
        const plots = await Plot.find({ projectId: req.params.projectId }).sort(
            { plotNumber: 1 },
        );
...
exports.getAllBuilderPlots = async (req, res) => {
    try {
        ...
        // Add pricePerSqft calculation to each plot
        const plotsWithPricePerSqft = plots.map((plot) => ({
            ...plot.toObject(),
            pricePerSqft: Math.round(plot.price / plot.sizeSqft),
        }));

        res.json({ plots: plotsWithPricePerSqft });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
};
...
exports.deletePlot = async (req, res) => {
    try {
        ...
        if (plot.status !== "available")
            return res
                .status(400)
                .json({ error: "Only available plots can be deleted" });
