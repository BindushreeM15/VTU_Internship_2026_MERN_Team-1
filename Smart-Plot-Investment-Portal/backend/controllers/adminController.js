...
exports.getPendingKYC = async (req, res) => {
    try {
        const builders = await SnipUser.find({
            role: "builder",
            kycStatus: "under_review",
        }).select(
            "name email phone companyName kycStatus kycDocuments kycSubmittedAt kycRejectionReason createdAt",
        );
        res.json({ builders });
    } catch (error) {
...
exports.getAllBuilders = async (req, res) => {
    try {
        const builders = await SnipUser.find({ role: "builder" }).select(
            "name email phone companyName kycStatus kycSubmittedAt kycVerifiedAt kycRejectionReason createdAt",
        );
        res.json({ builders });
    } catch (error) {
        res.status(500).json({ error: error.message });
...
exports.approveKYC = async (req, res) => {
    try {
        const builder = await SnipUser.findOne({
            _id: req.params.builderId,
            role: "builder",
        });
        if (!builder)
            return res.status(404).json({ error: "Builder not found" });
        if (builder.kycStatus !== "under_review")
            return res.status(400).json({ error: "Not under review" });
        ...
        await builder.save();
        res.json({
            message: `KYC approved for ${builder.companyName}`,
            kycStatus: "verified",
        });
    } catch (error) {
        res.status(500).json({ error: error.message });
...
exports.rejectKYC = async (req, res) => {
    try {
        ...
        await builder.save();
        res.json({
            message: `KYC rejected for ${builder.companyName}`,
            kycStatus: "rejected",
        });
    } catch (error) {
        res.status(500).json({ error: error.message });
...
exports.deleteBuilder = async (req, res) => {
    try {
        ...
        res.json({
            message: `Builder "${builder.companyName}" and all associated data deleted.`,
        });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
exports.getPendingProjects = async (req, res) => {
    try {
        const projects = await Project.find({ projectStatus: "under_review" })
            ...
            .select(
                "projectName location description amenities totalPlots projectStatus kathaType projectDocuments kathaDocument projectRejectionReason projectSubmittedAt builderId sketchImage projectImages",
            );
        res.json({ projects });
    } catch (error) {
        res.status(500).json({ error: error.message });
...
exports.rejectProject = async (req, res) => {
    try {
        ...
        await project.save();
        res.json({
            message: `"${project.projectName}" rejected`,
...
exports.activateProject = async (req, res) => {
    try {
        ...
        if (!["verified", "inactive"].includes(project.projectStatus)) {
            return res
                .status(400)
                .json({ error: "Cannot activate this project" });
        }
...
exports.deactivateProject = async (req, res) => {
    try {
        ...
        if (project.projectStatus !== "active") {
            return res
                .status(400)
                .json({ error: "Only active projects can be deactivated" });
        }
        project.projectStatus = "inactive";
        if (reason) project.projectRejectionReason = reason;
        await project.save();
        res.json({
            message: `"${project.projectName}" deactivated`,
...
exports.deleteProject = async (req, res) => {
    try {
        ...
        res.json({
            message: `"${project.projectName}" and all its plots deleted.`,
        });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
exports.getAllPlots = async (req, res) => {
    try {
        ...
        const formattedPlots = plots.map((p) => {
            ...
            const pricePerSqft = Math.round(p.price / p.sizeSqft);
            return {
...
exports.getAllInvestors = async (req, res) => {
    try {
        const investors = await SnipUser.find({ role: "investor" }).select(
